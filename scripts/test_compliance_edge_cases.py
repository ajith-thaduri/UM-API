...
def test_edge_cases():
    ...
    for case in test_cases:
        ...
        # Apply the same logic as in the service: sanitizer + overlap filter
        ...
        detected_types = [normalize_entity_type(r.entity_type) for r in results]
        
        passed = True
        
        # Check expected detections
        for entity in case["should_detect"]:
            norm_entity = normalize_entity_type(entity)
            if norm_entity not in detected_types:
                print(f"  FAILED: Expected {norm_entity} ({entity}), found {detected_types}")
                passed = False
        
        # Check unwanted detections
        for entity in case["should_not_detect"]:
            norm_entity = normalize_entity_type(entity)
            if norm_entity in detected_types:
                print(f"  FAILED: Did NOT expect {norm_entity} ({entity}), found it.")
                passed = False
