...
def classify_intent(question: str) -> str:
    ...
    if _GREETING_RE.match(q):
        return "greeting"
    if re.search(r"\bwho\s+(are|r)\s+you\b|\bwhat'?s\s+your\s+name\b", q):
    ...
    if re.search(r"\bsummarize the case\b|\bhigh.level overview\b|\bexecutive summary\b", q):
        return "case_summary"
    if re.search(
        r"\bwhere in the (chart|record|document)\b|\bpage\b|\bcite\b|\bevidence for\b|\bshow me (the )?source\b",
        q,
    ):
...
def _greeting_response(ctx: CaseAgentContextBundle) -> str:
    """Build a clean, contextual greeting — no pipeline jargon."""
    lines: List[str] = ["Hi! Ready to help with this case."]

    # One subtle context line — version + notable flags
    ctx_parts: List[str] = []
...
def _try_deterministic_answer(
    ...
    if intent == "greeting":
        return _greeting_response(ctx), None
...
def retrieval_policy(intent: str, question: str, used_deterministic: bool) -> Tuple[bool, str]:
    """Whether to run vector retrieval, and a short reason for the trace UI."""
    ...
    if intent in ("revision_diff", "compare_versions"):
        return False, "Compare uses case Context artifacts first"
    if intent == "contradictions":
        return False, "Contradictions come from case Context artifacts"
    q = question.lower()
    ...
    if intent == "case_summary":
        return False, "Case summary should come from case Context"
    if intent == "timeline":
        return False, "Start with case Context for timeline"
    if intent == "general_case_qa":
        return False, "Start with case Context; retrieve only if needed"
    return False, "Start with case Context; retrieve only if needed"


def should_retry_with_retrieval(intent: str, question: str, answer: str) -> bool:
    """
    After a case-context-first answer, decide whether to fall back to embeddings.
    This is intentionally conservative to avoid unnecessary retrieval.
    """
    if intent in {
        "greeting",
        "assistant_identity",
    ...
    q = question.lower()
    if re.search(r"\bpage\b|\bcite\b|\bsource\b|\bwhere in the\b|\bwhich document\b", q):
        return False
    ...
    return any(marker in a for marker in insufficiency_markers)
