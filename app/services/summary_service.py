...
class SummaryService:
            ...
            try:
                if not presidio_enabled:
                    ...
                    de_id_payload = {
                        ...
                        "document_chunks": document_chunks  # ENSURE CHUNKS INCORPORATED
                    }
                    vault_id = None
                ...
                if variables.get("document_chunks_text") and variables["document_chunks_text"] != "No raw document chunks available.":
                    safe_logger.info(f"Appending document chunks as context to Tier 2 prompt for case {case_id} (Presidio: {presidio_enabled})")
                    prompt_with_rules += "\n\n## PRIMARY SOURCE MATERIAL: DE-IDENTIFIED DOCUMENT CHUNKS\n"
                    if presidio_enabled:
                        prompt_with_rules += "CRITICAL: For HIPAA compliance and maximum accuracy, the following de-identified original document chunks are your PRIMARY source material. Use the synthesized sections above only as a contextual guide; prioritize the evidence found directly in these chunks.\n\n"
                    else:
                        prompt_with_rules += "The following document chunks provide additional raw clinical context. You should synthesize information from both the structured data provided above and these raw text chunks to create the most accurate summary.\n\n"
                    prompt_with_rules += variables["document_chunks_text"]
    ...
    async def generate_executive_summary(
        ...
        if enable_two_tier and db and case_id and user_id:
            ...
            try:
                ...
                # Format de-identified document chunks for supplement
                chunks = de_id_payload.get("document_chunks", [])
                if chunks:
                    safe_logger.info(f"Appending {len(chunks)} document chunks as context to executive summary prompt for case {case_id} (Presidio: {presidio_enabled})")
                    prompt += "\n\n## PRIMARY SOURCE MATERIAL: DE-IDENTIFIED DOCUMENT CHUNKS\n"
                    if presidio_enabled:
                        prompt += "CRITICAL: The following de-identified original document chunks are your PRIMARY source material. Use the data points provided above as a guide, but ensure your summary is grounded in the evidence found directly in these chunks.\n"
                    else:
    ...
    def _build_tier2_variables_from_payload(
        ...
        # Modular synthesis of text from clinical data
        # This provides a fallback if document chunks are missing, ensuring the prompt always has content.
        ...
        history_text = clinical_data.get("history_of_present_illness", "Not explicitly documented")
        social_text = clinical_data.get("social_history", "Not explicitly documented")
        therapy_text = clinical_data.get("therapy_notes", "Not explicitly documented")
        functional_text = clinical_data.get("functional_status", "Not explicitly documented")
        imaging_text = clinical_data.get("imaging", "Not explicitly documented")
        ...
        if chunks:
            ...
            chunks_formatted = "No raw document chunks available."
        ...
        return {
            ...
            "labs_abnormal_count": len(abnormal_labs),
            "timeline_count": len(timeline),
            "imaging_count": len(chunks) if chunks else 0,
            "document_chunks_text": chunks_formatted,
        }
